...
from .views import FitnessListView, FitnessDetailView

# any request getting through to this point is prefixed with the /fitness/ endpoint
...
urlpatterns = [
  path('', FitnessListView.as_view()), # as_view passes the http request onto the request attribute on the view/controller
  path('<int:pk>/', FitnessDetailView.as_view() )
